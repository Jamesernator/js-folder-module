...
function folderModule(directory, options = {}) {
    const outFile = typeof options.outFile === 'string' ? options.outFile : defaultOutFile(directory);
    const convertFilename = 'convert' in options ? options.convert : defaultConvert;
    const ignore = 'ignore' in options ? options.ignore : defaultIgnore;

    // Get rid of files that need to be ignored
    const inFiles = [...files(directory)].filter(fileName => !ignore(path.basename(fileName)));

    // Convert all filenames into
    const mappings = inFiles.map(fileName => ({
        fileName,
    ...
    const seen = new Map();
    mappings.forEach(({ fileName, varName }) => {
    ...
    const exportNodes = mappings.map(({ varName, fileName }) => ({
